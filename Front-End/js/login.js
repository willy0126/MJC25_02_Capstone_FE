...
/* ========================================
   공통 헬퍼 함수: 사용자 객체 생성
   @param {Object} userInfo - API에서 받은 사용자 정보
   @param {string|null} provider - 로그인 제공자 (일반 로그인: null, 소셜: 'SOCIAL')
   @returns {Object} 표준화된 사용자 객체
======================================== */
function createUserObject(userInfo, provider = null) {
    const user = {
    ...
    // 소셜 로그인인 경우 provider 추가
    if (provider) {
        user.provider = userInfo.provider || provider;
    }
...
function handleLoginSuccess(user) {
    ...
    setTimeout(() => {
        const returnUrl = getReturnUrl() || DEFAULT_RETURN_URL;
...
async function performLogin(email, password) {
    ...
    try {
        ...
        // 헬퍼 함수를 사용하여 로그인 처리
        const user = createUserObject(userInfo);
...
async function performSocialLogin(provider) {
    ...
    try {
        ...
        const currentReturnUrl = getReturnUrl() || DEFAULT_RETURN_URL;
...
function handleOAuthCallback() {
    ...
    // 일회용 코드 또는 에러 확인 (보안 강화: accessToken 대신 code 사용)
    ...
    if (isPopup) {
        ...
        try {
            const message = {
                type: 'OAUTH_CALLBACK',
                code: code || null,  // accessToken 대신 code 전달
                error: error || null,
            ...
            // 통신 실패 시 직접 처리 (폴백)
            handleOAuthCallbackDirect(code, error, errorMessage);
        }
...
/* ========================================
   OAuth 콜백 직접 처리 (폴백)
   일회용 코드를 서버에 전송하여 토큰으로 교환
======================================== */
async function handleOAuthCallbackDirect(code, error, errorMessage) {
    // URL에서 파라미터 제거 (깔끔한 URL 유지 - 보안상 중요)
...
async function exchangeCodeForToken(code) {
    try {
        ...
        // 일회용 코드를 서버에 전송하여 토큰으로 교환
        const response = await apiClient.exchangeOAuthCode(code);
        ...
        showToast(error.message || '소셜 로그인에 실패했습니다.', 'error');
    }
}
...
async function handleOAuthSuccess(accessToken) {
    try {
        ...
        // 헬퍼 함수를 사용하여 로그인 처리 (소셜 로그인)
        const user = createUserObject(userInfo, 'SOCIAL');
        handleLoginSuccess(user);
