document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Pollinations.ai로 이미지 생성 함수
    async function generateImageWithPollinations(prompt, index) {
        ...
        return new Promise((resolve, reject) => {
            ...
            img.onerror = () => reject(new Error(`이미지 생성 실패: ${index + 1}번째`));
            img.src = imageUrl;
        });
    }
    ...
    if (generatePicturebookBtn) {
        generatePicturebookBtn.addEventListener('click', async function() {
            ...
            document.getElementById('progressText').textContent = '그림책을 생성하고 있습니다... (1/5)';
            document.getElementById('progressFill').style.width = '0%';

            try {
                const imagesData = [];
                const totalRounds = Math.min(winnerStories.length, 4);
                const totalImages = totalRounds + 1; // 표지 1장 + 라운드별 이미지

                // 1. 먼저 표지 이미지 생성
                const coverPrompt = `children's picture book cover, cute style, colorful, fairy tale, ${currentPicturebookContest.title}`;
                document.getElementById('progressText').textContent = `표지 이미지 생성 중... (1/${totalImages})`;
                document.getElementById('progressFill').style.width = '0%';

                try {
                    const coverUrl = await generateImageWithPollinations(coverPrompt, 999);
                    imagesData.push({
                        round: 0,
                        imageUrl: coverUrl,
                        prompt: coverPrompt,
                    ...
                    console.log('표지 이미지 생성 완료:', coverUrl);
                } catch (coverError) {
                    console.error('표지 이미지 생성 실패:', coverError);
                    imagesData.push({
                        round: 0,
                        imageUrl: null,
                ...
                // 2. 각 라운드별 이야기를 기반으로 이미지 생성
                for (let i = 0; i < totalRounds; i++) {
                    const story = winnerStories[i];
                    const storyContent = story.story?.content || story.content || '';

                    // 이야기 내용을 기반으로 이미지 프롬프트 생성
                    const prompt = `children's picture book illustration, cute style, colorful, ${storyContent.substring(0, 200)}`;

                    document.getElementById('progressText').textContent = `${i + 1}차 이미지 생성 중... (${i + 2}/${totalImages})`;
                    document.getElementById('progressFill').style.width = `${((i + 1) / totalImages) * 100}%`;

                    try {
                        const imageUrl = await generateImageWithPollinations(prompt, i);
                        imagesData.push({
                            round: i + 1,
                            imageUrl: imageUrl,
                            prompt: prompt
                ...
                console.log('최종 이미지 데이터:', imagesData);
