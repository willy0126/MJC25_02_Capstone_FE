document.addEventListener('DOMContentLoaded', async function() {
    ...
    // 이미지 생성 작업 상태 폴링 함수
    async function pollImageGenerationStatus(jobId, maxAttempts = 60, interval = 3000) {
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            try {
                const status = await apiClient.getImageGenerationStatus(jobId);
                console.log(`이미지 생성 상태 (${attempt + 1}/${maxAttempts}):`, status);

                // 진행률 업데이트
                if (status.progress !== undefined) {
                    document.getElementById('progressFill').style.width = `${status.progress}%`;
                    document.getElementById('progressText').textContent =
                        `이미지 생성 중... (${status.progress}%)`;
                }
                ...
                if (status.status === 'failed' || status.status === 'FAILED') {
                    throw new Error(status.error || '이미지 생성에 실패했습니다.');
                }
    ...
    if (generatePicturebookBtn) {
        generatePicturebookBtn.addEventListener('click', async function() {
            if (!currentPicturebookContest) {
                showToast('대회 정보가 없습니다.', 'error');
                return;
            }

            const contestId = currentPicturebookContest.contestId;

            // 생성 시작
            generatePicturebookBtn.disabled = true;
            generateProgress.style.display = 'block';
            document.getElementById('progressText').textContent = '서버에 이미지 생성 요청 중...';
            document.getElementById('progressFill').style.width = '0%';

            try {
                // 1. 서버에 이미지 생성 요청
                console.log('이미지 생성 요청 시작, contestId:', contestId);
                const response = await apiClient.generateContestImages(contestId);
                console.log('이미지 생성 요청 응답:', response);

                const jobId = response.jobId || response.job_id || response.id;

                if (jobId) {
                    // 2. 비동기 작업인 경우 상태 폴링
                    document.getElementById('progressText').textContent = '이미지 생성 중... (서버에서 처리 중)';
                    document.getElementById('progressFill').style.width = '10%';

                    const result = await pollImageGenerationStatus(jobId);
                    console.log('이미지 생성 완료 결과:', result);
